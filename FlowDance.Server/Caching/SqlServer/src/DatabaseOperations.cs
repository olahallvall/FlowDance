...
internal sealed class DatabaseOperations : IDatabaseOperations
{
    ...
    public bool SetCacheItemOnce(string key, byte[] value, DistributedCacheEntryOptions options)
    {
        bool inserted = true;
        var utcNow = SystemClock.UtcNow;
        ...
        using (var connection = new SqlConnection(ConnectionString))
        using (var insertCommand = new SqlCommand(SqlQueries.SetCacheItemOnce, connection))
        {
