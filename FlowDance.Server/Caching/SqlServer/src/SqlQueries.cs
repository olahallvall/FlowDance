...
internal sealed class SqlQueries
{
    ...
    private const string SetCacheItemOnceFormat =
        "DECLARE @ExpiresAtTime DATETIMEOFFSET; " +
        "SET @ExpiresAtTime = " +
        "(CASE " +
                "WHEN (@SlidingExpirationInSeconds IS NUll) " +
    ...
    public SqlQueries(string schemaName, string tableName)
    {
        ...
        SetCacheItemOnce = string.Format(CultureInfo.InvariantCulture, SetCacheItemOnceFormat, tableNameWithSchema);
    ...
    public string SetCacheItemOnce { get; }
